...
import { createGame, tick, Game, Phase } from '../core/game';
import { FRAME_RATE } from '../core/tables';
import {
  Keyboard, classicFrame, horizontalFrame, mergeRaw, RawInput, Button, KEYMAP_P1, KEYMAP_P2,
} from './input';
import {
  GamepadInput, BIND_ORDER, BIND_LABELS, detectBind, padReleased, bindLabel, saveBinding, Binding,
} from './gamepad';
import { Audio } from './audio';
import {
  renderGame, renderHorizontal, renderVersus, renderMenu, renderConfig, ConfigRow, OverlayMenu, Mode, MENU_ROWS,
  CANVAS_W, CANVAS_H, H_CANVAS_W, H_CANVAS_H, V_CANVAS_W, V_CANVAS_H,
} from './render';
...
function resize(): void {
  if (screen === 'game' && menuMode === 'horizontal') { canvas.width = H_CANVAS_W; canvas.height = H_CANVAS_H; }
  else if (screen === 'game' && menuMode === 'versus') { canvas.width = V_CANVAS_W; canvas.height = V_CANVAS_H; }
...
function startGame(): void {
  ...
  keyboard1.snapshot(); keyboard2.snapshot(); gamepad1.snapshot(); gamepad2.snapshot();
  screen = 'game';
  resize();
}
...
function gotoConfig(player: number): void {
  configPlayer = player;
  cfgStep = 0; cfgPhase = 'wait'; cfgRest = null; cfgBinding = {};
  screen = 'config';
  resize();
}
function activateMenuRow(): void {
  switch (MENU_ROWS[menuSel]) {
    ...
    case 'config1': gotoConfig(1); break;
    case 'config2': gotoConfig(2); break;
...
function stepConfig(): void {
  ...
  keyboard2.snapshot(); gamepad1.snapshot(); gamepad2.snapshot(); // drain edges
  ...
  if (cfgPhase === 'saved') {
    if (kStart) { screen = 'menu'; resize(); return; }
    renderConfig(ctx, configPlayer, true, configRows(), 'Saved — all controls bound.', 'ENTER — done   ·   ESC — menu');
    return;
  }
  ...
  if (cfgPhase === 'wait') {
    ...
    if (padReleased(now)) { cfgRest = now; cfgPhase = 'capture'; }
  } else {
    ...
    if (bind) {
      ...
      if (cfgStep >= BIND_ORDER.length) {
        ...
        if (id) { saveBinding(id, cfgBinding); pad.reloadBinding(); }
        cfgPhase = 'saved';
      } else {
        cfgPhase = 'wait';
      }
  ...
  const prompt = cfgPhase === 'wait' ? 'Release, then press the next input…' : `Press the input for:  ${BIND_LABELS[action]}`;
...
function chooseOverlay(opt: string): void {
  ...
  else { shellPaused = false; screen = 'menu'; resize(); } // MAIN MENU
}
...
function stepSingle(): void {
  ...
  while (acc >= STEP_MS && steps < MAX_STEPS_PER_FRAME) {
    ...
    if (raw.pressed.start) { shellPaused = true; overlaySel = 0; acc = 0; return; } // open pause menu
    ...
    tick(g, input);
    if ((g.phase as Phase) === 'gameover') { overlaySel = 0; break; } // tick() may have ended it
    acc -= STEP_MS;
    steps++;
  }
...
function chooseVersusOverlay(opt: string): void {
  ...
  else { screen = 'menu'; resize(); } // MAIN MENU
}
...
function stepVersus(): void {
  ...
  while (acc >= STEP_MS && steps < MAX_STEPS_PER_FRAME) {
    ...
    if (r1.pressed.start || r2.pressed.start) { versusPaused = true; overlaySel = 0; acc = 0; return; }
    const i1 = classicFrame(r1); i1.startPressed = false;
    const i2 = classicFrame(r2); i2.startPressed = false;
    tick(g1, i1);
    tick(g2, i2);
    if (g1.phase === 'gameover' || g2.phase === 'gameover') {
      ...
      versusWinner = d1 && d2
        ? (g1.score >= g2.score ? 'PLAYER 1 WINS' : 'PLAYER 2 WINS')
        : d1 ? 'PLAYER 2 WINS' : 'PLAYER 1 WINS';
      overlaySel = 0;
      break;
    }
    acc -= STEP_MS;
...
function loop(now: number): void {
  ...
  if (screen === 'menu') {
    ...
    keyboard2.snapshot(); gamepad2.snapshot(); // drain P2 edges so none leak into a game
    if (k.up || p.up) menuSel = (menuSel + MENU_ROWS.length - 1) % MENU_ROWS.length;
    if (k.down || p.down) menuSel = (menuSel + 1) % MENU_ROWS.length;
    if (k.left || p.left) { if (menuSel === 0) cycleMode(-1); else if (menuSel === 1) levelDown(); }
    if (k.right || p.right) { if (menuSel === 0) cycleMode(1); else if (menuSel === 1) levelUp(); }
    ...
    const ov: OverlayMenu | undefined = versusWinner
      ...
      : versusPaused ? { title: 'PAUSE', options: PAUSE_OPTS, sel: overlaySel } : undefined;
    renderVersus(ctx, game, game2, ov);
  } else if (game) {
    stepSingle();
    audio.observe(game); // sound: single-player only
    const ov: OverlayMenu | undefined = game.phase === 'gameover'
      ? { title: 'GAME OVER', note: `SCORE ${String(game.score).padStart(6, '0')}`, options: OVER_OPTS, sel: overlaySel }
      : shellPaused ? { title: 'PAUSE', options: PAUSE_OPTS, sel: overlaySel, hideBoard: true } : undefined;
...
(window as unknown as { __dbg?: () => unknown }).__dbg = () => ({
  screen, menuMode, menuSel, menuLevel,
  phase: game?.phase, phase2: game2?.phase, versusWinner, versusPaused,
});
